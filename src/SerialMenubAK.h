...
template <typename SerialFileListing>
class SDMenuT : public menuNode, public SerialFileListing
{
public:
  ...
  SDMenuT(typename SerialFileListing::Type &sfl, constText *title, const char *at, Menu::action act = doNothing, Menu::eventMask mask = noEvent)
  ...
  void begin() {
    ...
    SerialFileListing::goFolder(folderName);
   }
    ...
    switch(event) {
      case enterEvent:
        if (nav.root->navFocus!=nav.target) {//on sd card entry
          nav.sel=((SDMenuT<SerialFileListing>*)(&item))->entryIdx(((SDMenuT<SerialFileListing>*)(&item))->selectedFile)+USE_BACKDOTS;//restore context
        }
    }
    return proceed;
  }
  ...
  void doNav(navNode& nav,navCmd cmd) {
    Serial.println("Do Nav");
    switch(cmd.cmd) {
      case enterCmd: if (nav.sel>=USE_BACKDOTS) {
          String selFile=SDMenuT<SerialFileListing>::entry(nav.sel-USE_BACKDOTS);
          if (selFile.endsWith("/")) {
            Serial.print("\nOpen folder...");
            //open folder (reusing the menu)
            folderName+=selFile;
            SDMenuT<SerialFileListing>::goFolder(folderName);
      ...
      case escCmd:
        if(folderName=="/")//at root?
          ...
          SDMenuT<SerialFileListing>::goFolder(folderName);
          dirty=true;//redraw menu
          nav.sel = SDMenuT<SerialFileListing>::entryIdx(fn) + USE_BACKDOTS;
        }
        return;
    }
  ...
  Used printTo(navRoot &root,bool sel,menuOut& out, idx_t idx,idx_t len,idx_t pn) {
    if(root.navFocus!=this) {//show given title or filename if selected
      ...
      _trace(Serial<<"closed menu! selected:"<<selectedFile.c_str()<<" idx:"<<idx<<endl);
      if (selectedFile=="") {
        _trace(Serial<<"menuNode::printTo...{"<<endl);
        return menuNode::printTo(root,sel,out,idx,len,pn);
        //return out.printRaw(shadow->text, len);
      } else {
        _trace(Serial<<"printRaw..."<<endl);
        return out.printRaw(selectedFile.c_str(),len);
      }
    } else if(idx==-1) {//when menu open (show folder name)
      ((menuNodeShadow *)shadow)->sz = SDMenuT<SerialFileListing>::count() + USE_BACKDOTS;
      idx_t at=folderName.lastIndexOf("/",folderName.length()-2)+1;
      String fn=folderName.substring(at,folderName.length()-1);
    ...
    else len-=out.printRaw(SDMenuT<SerialFileListing>::entry(out.tops[root.level]+idx-USE_BACKDOTS).c_str(),len);
    return len;
  }
};
